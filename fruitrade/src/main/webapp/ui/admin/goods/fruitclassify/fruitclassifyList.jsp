...
<%@page import="com.fruitrade.domain.FruitClassifyDo;"%>
<%
...
<html>
	...
	<body>
		...
		<div id="toolbar">
			<div id="addFruitClassify" class="btn btn-primary" data-toggle="modal" data-target="#myModal">新增水果分类</div>
			<div id="editFruitClassify" class="btn btn-success" data-toggle="modal" data-target="#myModal">编辑水果分类</div>
			<div id="deleteFruitClassify" class="btn btn-danger">删除水果分类</div>
	...
	<script type="text/javascript">
		...
		$('#addFruitClassify').click(function(){
			$("#winIframe").attr("src","addFruitclassify.jsp");
			$('#myModal').on('shown.bs.modal', function () {
				$(this).find('.modal-content').css('height','600px');// 修改modal的高度
				$(this).find('.modal-content').css('width','500px');// 修改modal的标题
				$(this).find('.modal-title').text('新增水果分类');// 修改modal的标题
			});
		});
		$('#deleteFruitClassify').click(function(){
			...
			if(confirm('确定删除吗？')){
				$.ajax({
					type: 'post',
					dataType: 'json',
					url: '/deleteFruitClassify.action',
					data: {'ids': ids},
					async: false,
					success: function(s){
						initTable('/listFruitClassify.action'); // 重新加载数据
					},
					error: function(e){
						alert("删除失败！");
					}
		...
		$('#editFruitClassify').click(function(){
			var row = $table.bootstrapTable('getSelections');
			if(row.length == 0){
				alert("请选择数据！");
				return false;
			...
			$("#winIframe").attr("src","/toUpdateFruitClassifyPage.action?id="+row[0].id);
			$('#myModal').on('shown.bs.modal', function () {
				$(this).find('.modal-content').css('height','600px');// 修改modal的高度
				$(this).find('.modal-content').css('width','500px');// 修改modal的标题
				$(this).find('.modal-title').text('编辑水果分类');// 修改modal的标题
			});
		});
		...
		$(".saveNow").click(function(){
			$.ajax({
				type: 'post',
				dataType: 'json',
				url: '/fruitclassify/saveOrUpdateFruitKind.action',
